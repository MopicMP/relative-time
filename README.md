...
[![npm version](https://badge.fury.io/js/relative-time.svg)](https://www.npmjs.com/package/relative-time)
...
// Basic usage
const result = time('your input here');
console.log(result);
```
...
```javascript
const { time, batch } = require('relative-time');

// Single item
const result = time('hello');
